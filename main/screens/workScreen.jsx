...
const ChapterInfoScreen = ({ route }) => {
  ...
  const translateWorkMeta = async workData => {
    try {
      ...
      const out = await translateTextsSmart(items);
  ...
  const handleTranslateLongPress = () => {
    if (translatingMeta) return;
    // 「本机 AI」需要有原生 Hymt 模块（Android 用官方 libai-chat.so，iOS 用自编的
    // llama.cpp + Metal）。这里用**运行时检测**而不是写死平台判断：哪个平台缺实现
    // 就不显示（避免用户点完拿到"本机模块不可用"这种误导提示），补齐后自动出现。
    const engineOptions = [
      {
        ...
        if (chapterToLoad && actualIndex !== -1 && actualIndex !== 0) {
          ...
          if (chapterContent) {
            const initialChapterData = {
              ...
              htmlContent: chapterContent.html,
              rawBody: chapterContent.body,
              rawCss: chapterContent.css,
