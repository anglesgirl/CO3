...
// A WebView image element cannot add X-Ech-Target. It uses this deliberately
// narrow loopback route instead; the native proxy validates the host again.
export async function echImageUrl(url) {
  const u = new URL(url);
  if (u.hostname !== 'images.weserv.nl') return url;
  const base = await requireEchBase();
  return base
    ? `${base}/__ech__/images.weserv.nl${u.pathname}${u.search}`
    : url;
}
...
export async function echRequest(url) {
  ...
  if (!PROTECTED_HOSTS.has(parsed.hostname)) return { url, headers: {} };
  const base = await requireEchBase();
