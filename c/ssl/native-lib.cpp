...
Java_com_liar_han1meplus_EchHttpClient_request(
  ...
  // 【关键修复 2026-09-11】禁止库自行跟随重定向：库只负责传输，重定向交回上层 OkHttp。
  // 原因（真机 + 服务端双重实证）：AO3 登录成功返回 302 + Set-Cookie: user_credentials，
  // 本库原先 FOLLOWLOCATION=1 会自己跳到 / 去，而 header_callback 在遇到新的 "HTTP/" 行时
  // 会 response->headers.clear()，把 302 那一跳的 Set-Cookie 全部清掉；又因本库没有 cookie jar，
  // 跟随的那一跳不带新 cookie，于是上层拿到的是「未登录首页」(200, 24468 字节)。
  // 结果：登录态永远同步不回 CookieManager，App 判登录必失败。
